/*For Android usage, the following permissions must be added:
   ...
   If you want to receive MSG_RESUMESESSION message, you must put setOnFileDownloaderListener before 
   initialization of your FileDownloader objects since the emit of MSG_RESUMESESSION message lays in
  ...
  #version 4.0	  @20140715
  	>>Enable listening to message in different classes;Using Static HandlerGroup instead of Singleton HandlerCenter.
  #version 4.1    @20140722
  	>>Introduce STATE RESUME which sends message to indicate resume download  
  	>>add public method reDownload()
  	>>policy declaration has been invoked in constructor
  #version 5.0    @20140723
  	>>Enable resume-download when user's app restarts
  	>>Improve implementation mechanism when successively press runDownload
   	>>Introduce STATE HAVEDONE which sends message to indicate file has been download already
   	>>Introduce STATE RESUMESESSION which sends message when download session is resumed(such as App restarts)
   	>>Add two RESUME callback arguments 
   	>>Changes cancelDownload method content to adapt to resume-download 
...
public class FileDownloader {
	...
	public static final int MSG_RESUMESESSION = 3;
	public static final int MSG_INPROGRESS = 4;
	public static final int MSG_SEGACCOMPLISH = 5;
	public static final int MSG_ACCOMPLISH = 6;
	public static final int MSG_PAUSED = 7;
	public static final int MSG_CANCELED = 8;
	public static final int MSG_HAVEDONE = 9;
	...
	public static abstract class onFileDownloaderListener {
		...
		public void onResumeSession(int downloaderId, int resumeStatusCode, int fileLength, int totalDoneSize){}
	...
	//constructor
	@SuppressLint("NewApi")
	public FileDownloader(Context context, String url, String path, int threadNum) {
	    StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
		StrictMode.setThreadPolicy(policy); 
		...
		if (dataStatus != FILE_NEVERDONE) {
			handlerGroup.sendMessageToAll( MSG_RESUMESESSION, pack_MSG_RESUMESESSION());
		}
	...
	{	
		downloadInfo_all = new DownloadInfo(id, -1, THREAD_NUM, fileLength, -1, fileLength, 0, 0, url);
		if (dataStatus == FILE_UNDONE_THREADNUM_IMPROPER || dataStatus == FILE_DONE_THREADNUM_IMPROPER || dataStatus == FILE_CLEAR) { //remove exist datum in db
			db.removeDownloadInfo(downloadInfo_all);
		}
		
		if (dataStatus == FILE_NEVERDONE || dataStatus == FILE_UNDONE_THREADNUM_IMPROPER || dataStatus == FILE_DONE_THREADNUM_IMPROPER) { //save
		...
		if (THREAD_NUM == 1) { //download with single thread
			...
			downloadInfo_seg = new DownloadInfo(id, 0, 0, fileLength, 0, fileLength, 0, 0, url);
			if (dataStatus == FILE_NEVERDONE || dataStatus == FILE_UNDONE_THREADNUM_IMPROPER || dataStatus == FILE_DONE_THREADNUM_IMPROPER) { //save
			...
			for (int i = 0; i < THREAD_NUM - 1; i++) {
				...
				downloadInfo_seg = new DownloadInfo(id, i, begPosition[i], endPosition[i], curPosition[i], segmentSize[i], 0, 0, url);
				if (dataStatus == FILE_NEVERDONE || dataStatus == FILE_UNDONE_THREADNUM_IMPROPER || dataStatus == FILE_DONE_THREADNUM_IMPROPER) { //save
					db.saveInfo(downloadInfo_seg);
				}
				else { //update
			...
			downloadInfo_seg = new DownloadInfo(id, THREAD_NUM-1, begPosition[THREAD_NUM-1], endPosition[THREAD_NUM-1], curPosition[THREAD_NUM-1], segmentSize[THREAD_NUM-1], 0, 0, url);
			if (dataStatus == FILE_NEVERDONE || dataStatus == FILE_UNDONE_THREADNUM_IMPROPER || dataStatus == FILE_DONE_THREADNUM_IMPROPER) { //save
				db.saveInfo(downloadInfo_seg);
			}
			else { //update
	...
	private static final int FILE_DONE_PROPER = 0;     //file has been downloaded and can be found in path
	private static final int FILE_DONE_THREADNUM_IMPROPER = 1;   //file has been downloaded but can not be found in path
	private static final int FILE_DONE_PATH_IMPROPER = 2;   //file has been downloaded but can not be found in path 
	private static final int FILE_UNDONE_PROPER = 3;   //file download unaccomplished and can be found in path
	private static final int FILE_UNDONE_THREADNUM_IMPROPER = 4; //file download unaccomplished but THREAD_NUM changes
	private static final int FILE_UNDONE_PATH_IMPROPER = 5; //file download unaccomplished but can not be found in path
	private static final int FILE_NEVERDONE = 6; //file never be downloaded 
	
	private static final int FILE_CLEAR = -1;
	...
	private int judgeDbDataStatus()
	{
		...
		if (info_all_get.is_done == 1) { //file has been downloaded 
			...
			else if (info_all_get.start_pos != THREAD_NUM) { //thread num changes
				return FILE_DONE_THREADNUM_IMPROPER;
			}
			else
			{
				return FILE_DONE_PATH_IMPROPER;
			}
		}
		else { //file download unaccomplished
			if (info_all_get.start_pos != THREAD_NUM) { //start_pos is used as thread_num for allInfo record
				return FILE_UNDONE_THREADNUM_IMPROPER;
			}
			else if (!file.exists()) {
				return FILE_UNDONE_PATH_IMPROPER;
			}
			else{
				...
				try {															
					...
					while (curPosition[tid] < endPosition[tid]) {
						...
						if (curPosition[tid] >= endPosition[tid]) {
							...
							downloadInfo_seg = new DownloadInfo(id, tid, begPosition[tid], endPosition[tid], curPosition[tid], segmentSize[tid], downloadSize[tid], 1, url);
							db.updateDownloadInfo(downloadInfo_seg);
							
							//update allInfo 
							downloadInfo_all = new DownloadInfo(id, -1, THREAD_NUM, fileLength, -1, fileLength, totalDownloadSize, 0, url);
							db.updateDownloadInfo(downloadInfo_all);
						...
						else
						{
							...
							downloadInfo_seg = new DownloadInfo(id, tid, begPosition[tid], endPosition[tid], curPosition[tid], segmentSize[tid], downloadSize[tid], 0, url);
							db.updateDownloadInfo(downloadInfo_seg);
					...
					if (totalDownloadSize == fileLength) {
						...
						isRunning = false;
						//update allInfo 
						downloadInfo_all = new DownloadInfo(id, -1, THREAD_NUM, fileLength, -1, fileLength, totalDownloadSize, 1, url);	
						db.updateDownloadInfo(downloadInfo_all);
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {		 
			    ...
			    case FileDownloader.MSG_RESUMESESSION:
			    	...
			    	listener.onResumeSession(downloaderId, resumeStatusCode, fileLen, totalDoneSize);
			    	break;
