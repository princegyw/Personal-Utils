...
public class FileDownloader {
	...
	private Class<?> callerCls = null; //caller's class
	...
	{	
		...
		HandlerCenter.sendMessage(callerCls, MSG_PAUSED, pack_MSG_PAUSED());
	}
	...
	public void cancelDownload()
	{
		...
		initSegmentData();
		HandlerCenter.sendMessage(callerCls, MSG_CANCELED, pack_MSG_CANCELED());
			...
			public void run() {
				...
				try {					
					...
					if (totalDownloadSize == 0) {
						HandlerCenter.sendMessage(callerCls, MSG_START, pack_MSG_START());
						LogUtil.v("id = " + id + "fileLen = " + fileLength);
					}
										
					//if tid seg start download
					if (downloadSize[tid] == 0) {
						HandlerCenter.sendMessage(callerCls, MSG_SEGSTART, pack_MSG_SEGSTART(tid));
					}
					...
					if (segmentSize[tid] < 0) {
						HandlerCenter.sendMessage(callerCls, MSG_ERROR, pack_MSG_ERROR("error segementSize"));
						return;
					}
					...
					while (curPosition[tid] < endPosition[tid]) {
						...
						if (curPosition[tid] >= endPosition[tid]) {
							...
							HandlerCenter.sendMessage(callerCls, MSG_INPROGRESS, pack_MSG_INPROCESS(tid) );
							HandlerCenter.sendMessage(callerCls, MSG_SEGACCOMPLISH, pack_MSG_SEGACCOMPLISH(tid));
						}
						else
						{
					...
					if (totalDownloadSize == fileLength) {
						...
						HandlerCenter.sendMessage(callerCls, MSG_ACCOMPLISH, pack_MSG_ACCOMPLISH());
					}
	...
	public static int unPack_MSG_id(Message msg)
	{
		return ((Bundle) msg.obj).getInt("id");
	}
	
	public static int unPack_MSG_START_fileLength(Message msg)
	{
		return ((Bundle) msg.obj).getInt("fileLength");
	}
	
	public static int unPack_MSG_SEGSTART_segId(Message msg)
	{
		return ((Bundle) msg.obj).getInt("segId");
	}
	
	public static int unPack_MSG_SEGSTART_segLength(Message msg)
	{
		return ((Bundle) msg.obj).getInt("segLength");
	}
	
	public static String unPack_MSG_ERROR_errInfo(Message msg)
	{
		return ((Bundle) msg.obj).getString("errInfo");
	}
	
	public static int unPack_MSG_INPROGRESS_curPos(Message msg)
	{
		return ((Bundle) msg.obj).getInt("curPos");
	}
	
	public static int unPack_MSG_INPROGRESS_begPos(Message msg)
	{
		return ((Bundle) msg.obj).getInt("begPos");
	}
	
	public static int unPack_MSG_INPROGRESS_endPos(Message msg)
	{
		return ((Bundle) msg.obj).getInt("endPos");
	}
	
	public static int unPack_MSG_INPROGRESS_segId(Message msg)
	{
		return ((Bundle) msg.obj).getInt("segId");
	}
	
	public static int unPack_MSG_INPROGRESS_totalDoneSize(Message msg)
	{
		return ((Bundle) msg.obj).getInt("totalDoneSize");
	}
	
	public static int unPack_MSG_SEGACCOMPLISH_segId(Message msg)
	{
		return ((Bundle) msg.obj).getInt("segId");
	}
